...
CalendarManager =
  makeUrl: (content) ->
    ...
    "&text=" + encodeURIComponent("#{content.name} [#{content.company}]") +
    "&details=" + encodeURIComponent("#{content.energy}kcal\n") +
    "&dates=" + CalendarManager.getUTC(new Date()) +
    "&trp=false" +
    "&sprop=" + encodeURIComponent('https://tanjoin.github.io/calorie-memo') +
    "&sprop=" + 'name:' + encodeURIComponent('calorie-memo')
    return url

  getUTC: (date) ->
    return date.getUTCFullYear() +
        CalendarManager.zerofill(date.getUTCMonth()+1) +
        CalendarManager.zerofill(date.getUTCDate()) +
        'T' +
        CalendarManager.zerofill(date.getUTCHours()) +
        CalendarManager.zerofill(date.getUTCMinutes()) +
        CalendarManager.zerofill(date.getUTCSeconds()) +
        'Z'
...
CalorieManager =
  convertToSalt: (sodium) ->
    return sodium * 2.54 / 1000

  convertToSodium: (salt) ->
  ...
  makeHtml: (content) ->
    ...
    sText.innerHtml +=
      "たんぱく質 #{content.protein}#{content.protein_unit}"
    sText.innerHtml += "<br>"
    sText.innerHtml +=
      "脂質 #{content.fat}#{content.fat_unit}"
    sText.innerHtml += "<br>"
    sText.innerHtml +=
      "炭水化物 #{content.carbohydrate}#{content.carbohydrate_unit}"
    sText.innerHtml += "<br>"
